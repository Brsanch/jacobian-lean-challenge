...
State: **1,218 `.lean` files**, **195,719 LOC** (+4,839 LOC across 60
new files + 3 manifest/docs edits). Full-graph build: **9,469 jobs**.
Zero `sorry`, zero `axiom`. Item count unchanged at **14 / 24
STRICT-CLOSED** (in `Basic.lean`); conditional item-flipping chain
available under `[HasPic0AnalyticEquiv X] + [Subsingleton ω]`.
Pushed to `origin/feat/c3-surface-classification-data`
(HEAD `2ed9a31`).
...
`axiom`. Item count unchanged at **14 / 24 STRICT-CLOSED** — the chip 19
arc is a *structural reduction* of the named classical content; items
5/11/12/13/17/18/21 cannot flip until the universal
`[HasJacobianAnalyticStructure X]` instance lands. Branch
`feat/c3-chip-19-iperiodform-hermitian`.

The chip 19 arc structurally closes the Hodge–Riemann second bilinear
relation. Reduces `CompleteHodgeRiemannHypothesis` from a
5-named-hypothesis bundle `(J, H, IsPositiveDefinite, first, bridge)`
to **(anti-sym `J` + first relation + matrix positivity)** —
eliminating the Hodge-form choice and the bridge identity as separate
atoms. At genus 1, reduces further to a SINGLE scalar inequality
(lattice orientation). Validates the chain on `T_L = ℂ ⧸ L`
end-to-end: `HasJacobianHodgeChain (ℂ ⧸ L)` from a single
`0 < Im(star lam₁ · lam₂)` input on the explicit in-tree basis.
...
* `Manifold/HodgeFormFromMatrix.lean` (161 LOC) — realise any
  Hermitian matrix `M` as a `HermitianOnHolomorphicOneForm X` via
  the sesquilinear sum `H om eta := ∑ i j, repr om i · M i j · star
  (repr eta j)`. Plus `toMatrix` recovery identity.

### Bridge identity + PD become automatic (chips c–e, 313 LOC)
...
* `Manifold/HodgeFormFromPeriodMatrixPD.lean` (144 LOC) — PD of the
  ...
  `H_can om om = star x ⬝ᵥ (M *ᵥ x)` with
  `x := star ∘ (basis_ω.repr om)`.
...
* `Manifold/CompleteHodgeRiemannFromStandardSymplectic.lean` (104 LOC)
  ...
  first-relation auto-discharge via the existing chip 13.
...
* `Manifold/CompleteHodgeRiemannGenusOneDiagonal.lean` (70 LOC) —
  1-input CHRH at genus 1 from
  `∀ i, ((i • Πᵀ J Π̄) i i).im = 0 ∧ 0 < .re`.
...
* `Manifold/PeriodMatrixFormStandardSymplecticOneSymbolic.lean`
  (107 LOC) — symbolic-`g` transport via `subst h_g` for `h_g : g = 1`.
...
* `Manifold/CompleteHodgeRiemannComplexTorus.lean` (140 LOC) —
  ...
  `0 < (star lam₁ · lam₂).im`. Uses
  `complexPeriod_torusBasisLoop_dz` + `basis_g_dz_apply` +
  `Subsingleton (Fin (genus T_L))` to identify periods with
  `lam₁, lam₂` on the reindexed symplectic basis.
...
* `Manifold/HasJacobianHodgeChainComplexTorusFromNonzero.lean`
  (57 LOC) — by-cases reduction from `Im ≠ 0` (combines the positive
  and swap-negative cases).

### Net

`HasJacobianHodgeChain (ℂ ⧸ L)` reduces from a 5-named hypothesis
bundle on a general genus-1 manifold to **a single nondegeneracy
condition**
