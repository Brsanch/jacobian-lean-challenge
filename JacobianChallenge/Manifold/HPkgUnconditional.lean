...
theorem h_pkg_holds_unconditional
  ...
  intro f hf hnc
  classical
  refine ⟨JacobianChallenge.Manifold.criticalValuesGeneral f,
    JacobianChallenge.Manifold.criticalValues_finite_general f hf hnc, ?_, ?_⟩
  · -- Witness-in-complement: replicate chip B's contradiction shape against
    ...
    have h_inj_F :
      ...
      JacobianChallenge.Manifold.injOn_nhds_of_deriv_ne_zero hFA_at_x h_deriv_ne'
  ...
  · -- Local constancy on Cᶜ via LocalSheetData supplier + HLcUnconditional.
    ...
    have h_sheets :
      ...
      have hy_not : y ∉ JacobianChallenge.Manifold.criticalValuesGeneral f := hy_compl
      have hx_eq : f x = y := hx
    ...
    exact h_lc_holds_for_subset_of_localSheets_supplier
      hf.continuous (JacobianChallenge.Manifold.criticalValuesGeneral f)
      h_fib h_sheets
