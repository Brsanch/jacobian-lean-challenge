...
matching the convention specified in challenge item 9 of `JacobianChallenge.Basic`:
...
`JacobianChallenge.Manifold.MeromorphicAt` (see its file header).
...
`JacobianChallenge.Manifold.degreeStub`. The sister edit that wires this in
will replace the `:= sorry` in `Basic.lean` with `:= degreeStub f hf`.

## Main definitions

* `JacobianChallenge.IsConstantMap f` — `f` is constant (one packaged form).
* `JacobianChallenge.Manifold.degreeStub f hf` — `0` if `f` is constant,
  `1` otherwise. **Stub.**
-/
