...
Companion to `JacobianChallenge.Manifold.AnalyticFiberDiscrete`. The
`regular_value_exists_of_critical_values_finite` reduction in
`JacobianChallenge.Manifold.Degree` requires, for each non-constant
...
lemma criticalSet_isDiscrete_of_not_constant
    ...
    (f : X → Y) (_hnc : ¬ JacobianChallenge.IsConstantMap f)
    (crit : Set X)
    (h : ∀ x ∈ crit, CriticalChartPullbackData f crit x) :
    IsDiscrete crit :=
  criticalSet_isDiscrete_of_chart_pullback f crit h
