...
/-- Auxiliary: `Germ.principalDivisorMap (1 : Germ X) = 0`. Reduces to
    the underlying `JacobianChallenge.principalDivisorMap_one` via the
    definitional `Germ.one_def` and the `Quotient.lift` computation rule
    `Germ.principalDivisorMap_mk`. -/
lemma germPrincipalDivisorMap_one :
...
/-- Auxiliary: `Germ.principalDivisorMap` intertwines multiplication on
    ...
    `JacobianChallenge.principalDivisorMap_mul`. -/
lemma germPrincipalDivisorMap_mul
  ...
  -- Induct on both germ classes using the tactic form, which infers the
  -- motive correctly relative to the existing `Germ X`-typed `g, h`.
  induction g using Quotient.inductionOn with
  | _ f =>
    induction h using Quotient.inductionOn with
    | _ f' =>
      -- Both `Quotient.mk _ f` and `MeromorphicNonzero.Germ.mk f` are
      -- definitionally equal at type `MeromorphicNonzero.Germ X`. The
      -- multiplication is the `Germ`-instance one; rewrite via
      -- `Germ.mk_mul` and `Germ.principalDivisorMap_mk`.
      change MeromorphicNonzero.Germ.principalDivisorMap
...
lemma PrincDivHonestCandidateGerm_eq :
  ...
  · -- `range principalDivisorAddHom ≤ closure (range principalDivisorMap)`.
    ...
    show MeromorphicNonzero.Germ.principalDivisorMap (Additive.toMul g)
            ∈ PrincDivHonestCandidate X
    -- Generalize the germ then induct on it.
    set q : MeromorphicNonzero.Germ X := Additive.toMul g with hq
    clear_value q
    ...
    | _ f =>
      ...
      change MeromorphicNonzero.Germ.principalDivisorMap (MeromorphicNonzero.Germ.mk f)
              ∈ PrincDivHonestCandidate X
      rw [MeromorphicNonzero.Germ.principalDivisorMap_mk]
...
/-! ## Honest `PrincDiv X` and `Pic0 X` (ZZ256, 2026-05-11)

Honest replacements for the placeholders that previously lived at the end
of `Divisor.lean`. They live here because the honest `PrincDiv X` references
`principalDivisorMap`, which would import-cycle through `Divisor.lean` if
defined there. Signatures gain
`[ChartedSpace ℂ X] [IsManifold (𝓘(ℂ, ℂ)) ω X]` over the prior topological
signatures. -/

/-- The honest **principal divisor subgroup** of `Div X`: the subgroup
generated by `(f) := principalDivisorMap f` for all `f : MeromorphicNonzero X`.
Definitionally equal to `PrincDivHonestCandidate X`. -/
noncomputable def PrincDiv (X : Type u)
    [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ChartedSpace ℂ X] [IsManifold (𝓘(ℂ, ℂ)) ω X] :
    AddSubgroup (Div X) :=
  PrincDivHonestCandidate X

/-- The honest **Picard group of degree-zero divisor classes** on a compact
complex 1-manifold. -/
noncomputable def Pic0 (X : Type u)
    [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ChartedSpace ℂ X] [IsManifold (𝓘(ℂ, ℂ)) ω X] : Type u :=
  Div0 X ⧸ (PrincDiv X).addSubgroupOf (Div0 X)

/-- `Pic0 X` is an additive commutative group. -/
noncomputable instance Pic0.instAddCommGroup (X : Type u)
    [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ChartedSpace ℂ X] [IsManifold (𝓘(ℂ, ℂ)) ω X] : AddCommGroup (Pic0 X) :=
  inferInstanceAs (AddCommGroup (Div0 X ⧸ (PrincDiv X).addSubgroupOf (Div0 X)))

/-- Generators of `PrincDiv X` lie in `PrincDiv X`. -/
lemma principalDivisorMap_mem_PrincDiv
    {X : Type u} [TopologicalSpace X] [T2Space X] [CompactSpace X]
    [ChartedSpace ℂ X] [IsManifold (𝓘(ℂ, ℂ)) ω X]
    (f : MeromorphicNonzero X) :
    principalDivisorMap f ∈ PrincDiv X :=
  principalDivisorMap_mem_PrincDivHonestCandidate f
