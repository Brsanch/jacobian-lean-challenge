...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE, Sub-chips 5.1 + 5.2 + 5.3{a,b,c} + 5.4{a,b,c-prep} LANDED**

Last rewrite: 2026-05-25 (Sub-chip 5.4c-prep landed — `∂̄_man v_i` agrees with the cutoff-free `chartPompeiuSolution` on `support(P.rhoC i)` via local-coincidence + germ-dependence of `partialZBarManifold`). Headlines —
...
Next: **Sub-chip 5.4c-final — compute `∂̄_man (chartPompeiuSolution i P α) y` in terms of α** for `y ∈ chart_xi.source`. The cutoff has been peeled off (5.4c-prep), so the remaining task is computing the manifold-side `∂̄` of `pompeiuKernel (chartPullbackZero …) ∘ chart_xi`. Strategy: apply Chip 4's `partialZBarManifold_pompeiuKernelChart_eq_α_mul_transition` (which handles `pompeiuKernelChart x α` with bare `α ∘ chart.symm`); bridge the `chartPullbackZero` vs bare-composition difference using that they agree on `chart_xi.target` (Sub-chip 5.3a's `chartPullbackZero_eqOn_target`) and `pompeiuKernel` integration only sees support, which for the bare composition is inherited from tsupport bounds. The result: `∂̄_man (chartPompeiuSolution) y · conj(deriv Φ_{y, i.val}(chart_xi y)) = (P.rhoC i * α) y`. Estimated 200-400 LOC, 2 sessions. **Open question**: whether the chart-transition factor cancels cleanly when summing over `i` (the partition-of-unity transformation rule for (0,1)-forms says yes, but encoding this in Lean is non-trivial). After 5.4c-final: 5.5 (Behnke-Stein spreading correction — the heavy one), 5.6 (assembly + manifold identity, where the transition-factor sum is the crux). Estimated remaining for Chip 5: ~550-1,200 LOC, 3-5 sessions.
