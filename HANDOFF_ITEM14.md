...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE, Sub-chips 5.1 + 5.2 + 5.3a + 5.3b LANDED**

Last rewrite: 2026-05-25 (Sub-chips 5.1 + 5.2 + 5.3a + 5.3b of Chip 5 landed — finite chart cover + subordinate smooth partition of unity + chart-pullback-extended-by-zero with full `HasCompactSupport` + `ContDiff ℝ ∞`). Headlines —
...
Next: **Sub-chip 5.3c — chart-x view local identity** for `u_i := pompeiuKernel (chartPullbackZero x_i (P.rhoC i * α))`. Build the global ℂ → ℂ Pompeiu solution from 5.3a/b: `HasCompactSupport (chartPullbackZero x_i (P.rhoC i * α))` (since `tsupport (P.rhoC i * α) ⊆ tsupport (P.rhoC i) ⊆ chart_xi.source`) plus `ContDiff ℝ ∞ (chartPullbackZero x_i (P.rhoC i * α))` give the inputs needed by Chip 3c-F-4. Conclude: for `y ∈ chart_xi.source`, `partialZBar u_i ((chart_xi) y) = chartPullbackZero x_i (P.rhoC i * α) ((chart_xi) y) = (P.rhoC i * α) y` via `chart_xi.left_inv`. Lift to an X-function via `u_i^X y := u_i ((chart_xi) y)` (well-defined for `y ∈ chart_xi.source`; arbitrary elsewhere — we'll cutoff in 5.4 anyway). Estimated 200-400 LOC, 1-2 sessions. After 5.3a/b/c: 5.4 (cutoff multiplication + error analysis), 5.5 (Behnke-Stein spreading correction — the heavy one), 5.6 (assembly + manifold identity). Estimated remaining for Chip 5: ~900-1,800 LOC, 4-7 sessions.
