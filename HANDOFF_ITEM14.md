...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE, Sub-chips 5.1 + 5.2 + 5.3{a,b,c} + 5.4{a,b} LANDED**

Last rewrite: 2026-05-25 (Sub-chip 5.4a + 5.4b landed — chart-source cutoff `χ_i` + global Pompeiu solution `v_i := (χ_i : ℂ) · pompeiuKernel … ∘ chart_xi` with global manifold smoothness and `tsupport ⊆ chart_xi.source`). Headlines —
...
Next: **Sub-chip 5.4c — manifold Leibniz for `∂̄_man v_i` + recovery on `supp(P.rhoC i)`**. Compute `partialZBarManifold v_i y` for `y ∈ chart_xi.source` using the manifold Leibniz rule `partialZBarManifold_mul` from `Manifold/PartialZBar.lean`. On `y ∈ supp(P.rhoC i)`: `χ_i y = 1` and `∂̄(pompeiuKernel … ∘ chart_xi)` at the chart-x view equals `(P.rhoC i * α) y` via Sub-chip 5.3c's `partialZBar_pompeiuKernel_chartPullbackZero_partition_mul_eq`; the `∂̄(χ_i)·(pompeiuKernel…)` term gets multiplied by χ_i = 1 but contributes via `∂̄χ_i = 0` on `supp(P.rhoC i)` (where χ_i is locally constant = 1). So `∂̄_man v_i = (P.rhoC i * α)` on `supp(P.rhoC i)`. Off `supp(P.rhoC i)`: `∂̄_man v_i` has the cutoff-derivative error term `∂̄χ_i · (pompeiuKernel…)`. Estimated 300-500 LOC, 2 sessions. **Subtlety**: the manifold-side `∂̄_man v_i` at `y ∈ chart_xi.source` uses `chart_y` (chart AT y), generally different from `chart_xi`; need a chart-transition factor (mirror of Chip 4's `partialZBarManifold_pompeiuKernelChart_eq_α_mul_transition`). After 5.4c: 5.5 (Behnke-Stein spreading — the heavy one), 5.6 (assembly + manifold identity). Estimated remaining for Chip 5: ~750-1,400 LOC, 4-6 sessions.
