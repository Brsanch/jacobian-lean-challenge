...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE, Sub-chips 5.1 + 5.2 + 5.3a + 5.3b + 5.3c LANDED**

Last rewrite: 2026-05-25 (Sub-chip 5.3 trilogy complete — chart-pullback infrastructure + global smoothness + chart-x view local identity for the chart-local Pompeiu solution). Headlines —
...
Next: **Sub-chip 5.4 — cutoff multiplication + error analysis**. Introduce a second smooth cutoff `χ_i : X → ℝ` with `χ_i ≡ 1` on a neighborhood of `supp (P.rhoC i)` and `tsupport χ_i ⊆ (chartAt ℂ i.val).source`; form `v_i := (χ_i : ℂ) · u_i^X` where `u_i^X y := pompeiuKernel (chartPullbackZero i.val (P.rhoC i * α)) ((chart_xi) y)` for `y ∈ chart_xi.source` (and 0 / unspecified elsewhere — `χ_i` cuts it off). Then `v_i` is globally smooth + tsupport ⊆ chart_xi.source. Compute `∂̄ v_i = χ_i · ∂̄ u_i^X + ∂̄ χ_i · u_i^X` via the manifold Leibniz rule (`partialZBarManifold_mul`). On `supp (P.rhoC i)`: `χ_i = 1` and `∂̄ u_i^X = (P.rhoC i * α)` (from 5.3c), so `∂̄ v_i = ρ_i · α` plus error `∂̄ χ_i · u_i^X` supported away from `supp(P.rhoC i)`. Summing over `i` and using `Σ ρ_i = 1`: `Σ ∂̄ v_i = α + e` where `e := Σ ∂̄χ_i · u_i^X`. Sub-chip 5.5 (Behnke-Stein spreading) handles the iterative cancellation of `e`. Estimated 300-500 LOC, 2 sessions. After 5.4: 5.5 (Behnke-Stein — the heavy one), 5.6 (assembly + manifold identity). Estimated remaining for Chip 5: ~750-1,400 LOC, 4-6 sessions.
