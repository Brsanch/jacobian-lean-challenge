...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE; Chip 5 structural backbone (5.1 + 5.2 + 5.3{a,b,c} + 5.4{a,b,c-prep,c-final}) + assembly layer (combined per-i identity, basepoint identity, globalSolutionCandidate, finsetSum distributivity, ∂̄_man-of-global = Σ ∂̄_man v_i assembly, zero-off-tsupport-χ, partition-times-α target) LANDED**

Last rewrite: 2026-05-25 (assembly layer landed — 7 chips beyond the 5.4 trilogy that wire the per-i identities into a global Σ_i framework, ship the trivial vanishing case off-tsupport, and ship the target partition-of-unity-times-α identity). Headlines —
...
Next: **Sub-chip 5.5 — Behnke-Stein spreading correction**. Now that Sub-chip 5.4 trilogy delivers `partialZBarManifold v_i = (P.rhoC i * α) / conj(transition factor)` on `support(P.rhoC i)` (combining 5.4c-prep + 5.4c-final), Sub-chip 5.5 handles the error term `∂̄ v_i - ρ_i · α` arising from the cutoff χ_i's non-trivial derivative on `tsupport χ_i \ supp(ρ_i)` (the annulus where χ_i transitions from 1 to 0). Strategy: form the **global candidate** `u := Σ_i v_i / conj(transition_i)` (where the transition factor cancels in the partition-of-unity sum by the (0,1)-form transformation rule). The deficit `α - ∂̄_man u` is supported in the union of cutoff annuli, smaller than the original α. Iterate (Forster Ch. 14): re-solve `∂̄ w = α - ∂̄_man u`, add to u, repeat. On genus-0 X this iteration converges (Behnke-Stein spreading-function lemma). Estimated 600-1,000 LOC, 3-5 sessions. **Open**: the (0,1)-form transformation rule encoding requires either (a) directly proving the transition-factor cancellation in the partition sum or (b) reformulating v_i with the inverse transition factor baked in. After 5.5: 5.6 (final manifold identity assembly). Estimated remaining for Chip 5: ~750-1,300 LOC, 3-6 sessions.
