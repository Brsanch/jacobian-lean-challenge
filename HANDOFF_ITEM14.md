...
## 🟢 ACTIVE ARC: Pompeiu kernel (committed 2026-05-24) — **Chip 3c-F + Chip 4 COMPLETE, Sub-chips 5.1 + 5.2 + 5.3a LANDED**

Last rewrite: 2026-05-25 (Sub-chips 5.1 + 5.2 + 5.3a of Chip 5 landed — finite chart cover + subordinate smooth partition of unity + chart-pullback-extended-by-zero with HasCompactSupport). Headlines —
...
* **`chartPullbackZero x α : ℂ → ℂ`** + **`hasCompactSupport_chartPullbackZero`** ([`Manifold/ChartPullbackExtendZero.lean`](JacobianChallenge/Manifold/ChartPullbackExtendZero.lean), 252 LOC, Sub-chip 5.3a): `Set.indicator`-based extension `α ∘ chart_x.symm` to a globally-defined ℂ → ℂ function (zero outside `chart_x.target`), with **`HasCompactSupport`** under `[CompactSpace X]` + `tsupport α ⊆ (chartAt ℂ x).source`. This handles the structural obstruction that `chartAt ℂ x).symm` is junk outside `chart_x.target` and the bare composition `α ∘ chart_x.symm` need not be compactly supported. Smoothness (`ContDiff ℝ ∞`) is deferred to Sub-chip 5.3b.
...
Next: **Sub-chip 5.3b — chart-pullback smoothness bridge**. Show `ContDiff ℝ ∞ (chartPullbackZero x α)` when `α : X → ℂ` is `ContMDiff 𝓘(ℝ, ℂ) 𝓘(ℝ, ℂ) ∞` and `tsupport α ⊆ chart_x.source`. Strategy: glue smoothness across the open cover `{chart_x.target, ℂ \ (chartAt ℂ x) '' (tsupport α)}` via mathlib's `contMDiffOn_chart_symm` bridge for manifold-side α to chart-side smoothness, plus that the extension is identically zero on a neighborhood of `ℂ \ chart_x.target`. Estimated 300-500 LOC, 2 sessions. **Sub-chip 5.3c** then defines `u_i^chart := pompeiuKernel (chartPullbackZero x_i (P.rhoC i * α))` and applies Chip 3c-F-4 (`partialZBar_pompeiuKernel_eq_self`) plus `chart.left_inv` to get the chart-x view local identity on `chart_xi.source`. After 5.3a/b/c: 5.4 (cutoff multiplication + error analysis), 5.5 (Behnke-Stein spreading correction — the heavy one), 5.6 (assembly + manifold identity). Estimated remaining for Chip 5: ~1,200-2,300 LOC, 5-9 sessions.
