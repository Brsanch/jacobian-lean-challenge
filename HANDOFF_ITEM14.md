...
### Sub-chip 5.5a — chart-anchored ∂̄ operator (Path A fork resolution, 2026-05-25)
...
Next: **Sub-chip 5.5b — re-state per-`i` recovery identity in the anchored frame**. Restate the combined per-i identity `partialZBarManifold_localPompeiuSolutionGlobal_eq_α_mul_transition_on_support_rhoC` ([`Manifold/LocalPompeiuSolutionManifoldIdentity.lean`](JacobianChallenge/Manifold/LocalPompeiuSolutionManifoldIdentity.lean)) in the chart-anchored frame: on `support(P.rhoC i)`, `partialZBarManifoldAtChart i.val v_i y = (P.rhoC i * α) y` — no transition factor on the RHS. The proof should reduce to Chip 5.4c-prep (∂̄_man v_i = ∂̄_man chartPompeiuSolution on support(P.rhoC i), germ argument carries through to the chart-anchored op identically via the `EventuallyEq.fderiv_eq` lemma already used) plus the chart-x view collapse from `partialZBar_chartPompeiuSolution_chart_xi_symm_eq` in `ChartPompeiuSolutionManifoldIdentity.lean`. Estimated 100–200 LOC. After 5.5b: 5.5c re-states the global candidate's ∂̄ identity (`partialZBarManifoldAtChart i.val (Σ_j v_j) y = Σ_j partialZBarManifoldAtChart i.val v_j y` + per-`i` evaluation), 5.5d handles the cutoff annulus error term (the actual Behnke-Stein spreading), 5.5e closes the global solution. Estimated remaining for Chip 5: ~500–900 LOC, 3–5 sessions.
