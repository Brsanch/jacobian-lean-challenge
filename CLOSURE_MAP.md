...
| **C3 (chain-level + algebra + per-generator)** | `AbelHypothesis B ← AbelChainPeriodCondition B ← AbelGeneratorPeriodCondition B` (`Manifold/AbelHypothesisFromPeriodCondition.lean`, ~525 LOC across 4 commits 2026-05-14). Reduces C3 to a single sharp atomic statement: for each `f : MeromorphicNonzero X`, the period vector of the AJ chain of `div(f)` lies in `periodLatticeImage`. **9-step staircase landed 2026-05-15** (~2,582 LOC across 15 files; steps 1–9 all have chips, see `OPEN.md` for per-step file list). Cycle-period argument fully proven in-tree; ∂(levelSetChain) = -principalDivisorMap f established pointwise via order-1 zero / order-(-1) pole analysis using chart-independence of MMeromorphicAt. **2026-05-16:** step-9's `h_AJ_boundary` companion hypothesis discharged unconditionally (`Manifold/PrincipalDivisorAJChainBoundary.lean`, ~125 LOC; ℤ-linearity of `boundary` on the explicit `Σ D(x) • single (pathFromBase x)` sum + `JacobianChallenge.residue_theorem` to kill the basePoint term at `D.degree = 0`). β-existence for `h_struct`'s boundary clause discharged (`Manifold/MeromorphicNonzeroRegularPath.lean`, ~431 LOC; `exists_regular_path_zero_to_infty` via two `linearInChartSegment` paths through bridge point `s + i` for generic `s`, concat'd via `SmoothPath.concat`, ambient extracted as ℝ → RS smooth function — produces β with `β 0 = 0`, `β 1 = ∞`, `β t ∈ regularValueSet f` on `[0, 1]` under `0 ∈ regularValueSet` and `∞ ∈ regularValueSet` hypotheses). | **~600–1,200** *remaining* (residual analytical content: explicit `f_*ω` SmoothOneForm construction + Stokes/residue argument to discharge `h_struct`'s lattice clause; β existence is now in tree) |
