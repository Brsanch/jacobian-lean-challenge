...
| **C3 (chain-level reduction)** | `AbelChainPeriodCondition B → AbelHypothesis B` (`Manifold/AbelHypothesisFromPeriodCondition.lean`, ~210 LOC). Reduces C3 to a single textbook-citable condition: period vectors of explicit AJ chains lie in `periodLatticeImage`. | **1,000–2,500** *remaining* (general-genus discharge of `AbelChainPeriodCondition`) |
...
3. **C3 `AbelHypothesis` + C4 `JacobiInversion` (~2,400–5,600 combined; ~2,200–5,300 *remaining* after 2026-05-14 work).** Completes `Pic⁰ ≃+ AnalyticJacobian` unconditionally. **Highest-leverage chunk: flips items 4, 5, 10, 11, 12, 13 simultaneously (6 items)** via the existing bundle. *(Two C3 pieces landed 2026-05-14: genus-0 corner via subsingleton AnalyticJacobian — `Manifold/AbelHypothesisGenusZero.lean`; chain-level reduction to a single textbook-citable period condition — `Manifold/AbelHypothesisFromPeriodCondition.lean`. The Stokes-on-2-chains content at genus ≥ 1 remains as discharging `AbelChainPeriodCondition` for arbitrary principal divisors.)*
