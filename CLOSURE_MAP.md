...
## F. Net realistic ceiling at this pin (revised 2026-05-18 late after concat-additivity + V-loop-bounds + structural-reduction arcs)

### F.0 Measured ground truth (latest 2026-05-18 late)

- **Total `.lean`:** **140,066 LOC across 781 files**, full `lake build` at **9061 jobs** (measured 2026-05-18 late, origin/main HEAD `5ca33ec`). Net delta from the prior 119,776 / 635-file baseline (2026-05-16 late) reflects:
  - **2026-05-17/18 (canonical Stokes bundle + path-plus-reverse + Hodge wave + symplectic bundle migration)** — ~7,820 LOC across 56 chips.
  - **2026-05-18 late (concat-additivity + rebasing + V-loop-bounds + structural reduction)** — ~4,400 LOC across 22 chips (HEAD `5ca33ec`). The capstone reduces `stokesBoundaries 𝓘(ℝ, ℂ) RiemannSphere = ⊤` (the load-bearing genus-0 input for canonical period-lattice closure) to TWO atomic named predicates: `SmoothLoopAvoidsInftyHypothesis p₀` + `SmoothLoopChartNPullbackExistsHypothesis p₀`. The two predicates remain as future arcs (Sard / measure-zero of smooth-curve image + Möbius push; mathlib `ContDiffBump` tubular-neighborhood smooth-pullback).
- **2026-05-18 late wave (22 chips, +4,350 LOC):** concat 2-simplex with `face1 = γ.concat δ` (3 chips); reparam-invariance for bumpedHalfLeft / bumpedHalfRight (6 chips via two-triangle square-diagonal split); concat-additivity capstone `single (γ.concat δ h) - single γ - single δ ∈ stokesBoundaries` (1 chip); triple-concat / rebasing / loop-rebasing (3 chips); `BasedSmoothLoopsBoundHypothesis` named-predicate + reduction (1 chip); V-loop-bounds unconditional on normed ℝ-vector spaces (3 chips); `Smooth2Simplex.push` + `Smooth2Chain.push` + `boundary₂_push` + `stokesBoundaries_push` (1 chip); composition + chart-factorisation discharge (2 chips); chartN/chartS.symm smoothness in `𝓘(ℝ, ℂ)` (1 chip); structural reduction to the two atomic predicates (1 chip). Build delta 9039 → 9061 jobs. Zero `sorry`, zero `axiom`.
- **2026-05-17 wave (16 chips, +2,948 LOC):** Hodge finite-dim Forster scaffolding through `HolomorphicOneForm X` packaging — `localCoeff` API (340 + 338 = 678), `DiskChartCover` + seminorm (201 + 252 + 119 = 572), Cauchy estimate + Lipschitz bound (204 + 154 = 358), Arzelà-Ascoli + diagonal (188 + 114 = 302), pointwise limits + CLM-level lift (113 + 192 = 305), analyticity (173), section construction (79 + 188 + 109 + 184 = 560). Build delta 8786 → 8802 jobs (pre-rebase). **Net effect**: full Forster proof reduced to two remaining steps — seminorm convergence (inner→outer via multi-chart density) + Riesz application with `NormedAddCommGroup` + separating.
- **2026-05-16-evening wave (13 chips, +2,280 LOC, `feat/abel-generator-input-independence` FF-merged):** C3 structural reduction + chain-rule pathway segments 1-3. AbelGenerator predicates `AbelJacobiInput`-invariant; C3 reduces to 2 named classical inputs `RegularLevelSetLatticeClause` + `AbelLatticeWitnessCriticalCase`; per-`t` structural chain-rule identity on `Ioo 0 δ`; uniform-δ packaging; generalised sourceFiberPath injectivity at any `t ∈ Icc 0 1` (injection half of bijection). Build delta 8793 → 8808 jobs.
...
- **Cumulative net delta 2026-05-14 → 2026-05-17 + 2026-05-16-evening:** +104 files / +17,134 LOC across all seven arcs.
- **Scoreboard:** 13 STRICT-CLOSED · 2 STUB · 9 OPEN (item flips await Phase 2 wiring of the Abel-Jacobi iso on arbitrary `X`, not the genus-0 corner).
...
| **B** | `HolomorphicOneFormFiniteDim X` — **2,948 LOC scaffolding landed 2026-05-17** via the elementary Forster/Montel/Riesz route (NOT L² Hodge): `localCoeff` API + `DiskChartCover` + seminorm + Cauchy/Lipschitz + Arzelà-Ascoli + diagonal subseq + pointwise CLM limit + analyticity + `limitHolomorphicOneForm` packaging (`Manifold/DiskChartCover*.lean` + `Manifold/HolomorphicOneFormChartCoeff*.lean`). Remaining: seminorm convergence upgrade (inner→outer via multi-chart density bound) + Riesz application (`NormedAddCommGroup` wrapper + separating from cotangent invertibility + `FiniteDimensional.of_isCompact_closedBall₀`). | **~1,000–1,800** *remaining* |
...
| **C3 (chain-level + algebra + per-generator)** | `AbelHypothesis B ← AbelChainPeriodCondition B ← AbelGeneratorPeriodCondition B` (`Manifold/AbelHypothesisFromPeriodCondition.lean`, ~525 LOC across 4 commits 2026-05-14). Reduces C3 to a single sharp atomic statement: for each `f : MeromorphicNonzero X`, the period vector of the AJ chain of `div(f)` lies in `periodLatticeImage`. **9-step staircase landed 2026-05-15** (~2,582 LOC across 15 files; steps 1–9 all have chips, see `OPEN.md` for per-step file list). Cycle-period argument fully proven in-tree; ∂(levelSetChain) = -principalDivisorMap f established pointwise via order-1 zero / order-(-1) pole analysis using chart-independence of MMeromorphicAt. **f_*ω stack landed 2026-05-16** (~1,573 LOC across 10 chips): step-9's `h_AJ_boundary` companion hypothesis discharged (`PrincipalDivisorAJChainBoundary.lean`, ~125 LOC); β-existence for `h_struct`'s boundary clause (`MeromorphicNonzeroRegularPath.lean`, ~431 LOC); concrete `regularLevelSetChain` + boundary identification (~146 LOC); real-model RS manifold + open-set realification (~96 LOC); pointwise cotangent pullback primitive (~94 LOC); pointwise trace `f_*ω` at a regular value (~117 LOC); `SmoothOneFormOn` partial-section type + `restrictOnSet` (~88 LOC); scalar evaluation of cotangent pullback + trace (~123 LOC); `ContinuousOn` variant of `path_lift_eqOn_Icc` (~131 LOC); local identification `sourceFiberPath ↔ sheet.g ∘ β ∘ σ` (~222 LOC). **C3 structural-reduction + chain-rule pathway segments 1-3 landed 2026-05-16 evening** (~2,280 LOC across 13 chips, `feat/abel-generator-input-independence`, FF-merged at HEAD `9a9d45c`): (i) `AbelGenerator*` is `AbelJacobiInput`-invariant via the boundary-of-AJ-chain-diff-is-cycle argument plus `residue_theorem` for principal divisors; (ii) `AbelHypothesis B` reduces to the named `AbelLatticeWitness X α h`; (iii) `AbelLatticeWitness` splits into `RegularLevelSetLatticeClause` + `AbelLatticeWitnessCriticalCase` via a public `toFun`↔`toRS` constancy bridge; (iv) per-`t` structural chain-rule identity `∑_p integrand(sourceFiberPath p) ω t = applyCotangent (∑_p cotangentPullbackAt sheet_p.g (β(σ t)) ω) (β'(σ t) σ'(t))` (chain rule + applyCotangent distribution); (v) uniform-δ packaging across sourceFiber via `Finset.min'`; (vi) **generalised injectivity of `sourceFiberPath.toPath.extend` at any `t ∈ Icc 0 1`** (injection half of the sourceFiber ↔ `f⁻¹(β(σ t))` bijection). **C3 → 2 named classical inputs**: `RegularLevelSetLatticeClause` (substantive analytic core: chain-rule global gluing + bijection surjectivity + `f_*ω` residue theorem on `ℙ¹`) and `AbelLatticeWitnessCriticalCase` (small Möbius-substitution residual). | **~1,500–3,000** *remaining* (surjectivity of the bijection via cardinality, Lebesgue gluing across the cover, σ-reparametrisation, and the residue theorem for meromorphic 1-forms on `ℙ¹` — the last is the genuine analytic content) |
...
**Final-state projection** (23/24 path): repo grows from current **119,776 LOC** → **~131,600–147,000 LOC** (adding the §F.3 11,800–27,200 LOC remaining range).
...
6. **B `HolomorphicOneFormFiniteDim`** — **2,948 LOC of scaffolding landed 2026-05-17** (Forster/Montel/Riesz route, not L² Hodge). Remaining ~1,000–1,800 LOC: seminorm convergence + Riesz application. Flips item 1.
