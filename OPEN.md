...
> **2026-05-23 holomorphic parametric integral arc closure: `ChartLocalPrimitiveSmoothExt` at `chartAt ℂ y` UNCONDITIONAL (6 chips, +1,376 Lean LOC across 6 new files; all local on `main`, not pushed). Repo now 1,072 `.lean` files / 182,183 LOC. Per-file `lake env lean` clean; umbrella `lake build` still 9,326 jobs (new chips not yet wired in). Item count in `Basic.lean` unchanged: **14/24 STRICT-CLOSED**. The chips factor item 14's `h_smooth_b` to *unconditional* at the natural per-point chart cover, but item 14 itself still requires the remaining `h_ftc_b` + classical inputs `hSP` + `h_bslb` to flip. See CHANGELOG.md for chip-by-chip breakdown (A: AnalyticOn of chart-coord param integral; B1+B2+B3: chart-pulled identity; C: ChartLocalPrimitiveSmoothExt; D1: FTC atom).**
>
> **2026-05-21 post-PR-#4 continuation (FTC-arc foundation, ℂ→ℝ diamond bypass to `ContDiffOn ℝ 1`, holomorphic parametric integral atom; 11 chips landed directly on `main`; +907 Lean LOC across 11 new files). origin/main HEAD `fe55640`. Repo at 1066 `.lean` files / 180,807 LOC at that point. Build 9326 jobs clean. Item count unchanged: **14/24 STRICT-CLOSED**. See CHANGELOG.md for sub-arc breakdown (A: integrand smoothness; B: ℂ→ℝ diamond bypass; C: holomorphic parametric integral). Big-picture pivot: the actual `h_smooth_b` blocker is `ContMDiffOn ω` (= holomorphic), not real-smoothness — sub-arc C is the direct path; sub-arc B is useful foundational infra but tangential.**
>
...
>
> **Architectural blocker identified for the final concatenation chip**: combining sub-arc bordisms requires each subpath's `ambient` to be **globally** chart-contained (∀ t : ℝ), but `γ.subpath.ambient` outside `[0, 1]` is `Classical.choose`-determined. Investigation revealed bump-extension via smooth reparam σ : ℝ → [0, 1] with σ = identity on [0, 1] is **mathematically impossible** (σ'(0⁻) = σ'(0⁺) = 1 forces σ < 0 just left of 0, contradicting σ ≥ 0). The genuine fixes are: (1) restructure `Smooth2Simplex` to use `ContMDiffOn [0, 1]²` (major refactor); (2) explicit bump extension via `ContDiffBump` exposing the ambient (needs a finer SmoothPath structure); (3) sidestep with strong-hypothesis pattern for specific X with finite chart cover.
