...
| 14 | `genus_eq_zero_iff_homeo` (anti-hack vs. `genus := 0`) | **OPEN** | `sorry`. Architecturally closed via zz331's `genus_eq_zero_iff_homeo_from_all_conditionals` ([Topology/Item14FinalComposition.lean](JacobianChallenge/Topology/Item14FinalComposition.lean)). After zz302–zz350 (~5.0k+ LOC across two sessions): the **`RiemannRochGenusZero X` input has been reduced to a single deeper open hypothesis** `ExistsNonConstantBoundedByDeltaP_GenusZero X` (see zz337–zz350). The analytic bridge (`MeroSinglePoleExtendsToDeg1Map` via the simple-pole-to-degree-1-map construction), the universal regularity at simple poles (`UniformSimplePoleRegularity`), and `LiouvilleOnCompactConnected` are all now THEOREMS (zz344, zz350). The remaining open inputs for item 14: (1) `ExistsNonConstantBoundedByDeltaP_GenusZero X` (the `dim L(δp) ≥ 2` Riemann-Roch + Serre-duality content at genus 0), (2) `Surjective_of_NonConstant_Analytic_Manifold`, (3) `BijectiveAnalyticIsBiholomorphism`, (4) topological-sphere-uniformization branch. See CLOSURE_MAP.md §D.2 for the full per-input LOC table. |
